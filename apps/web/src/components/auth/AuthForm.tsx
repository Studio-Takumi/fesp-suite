...
type AuthFormProps = {
    mode: 'login' | 'signup'
    /** ログイン後に戻るパス（`?redirect=` の値）。確かめる前の値をそのまま渡してよい */
...
const TEXT = {
    login: {
        title: 'ログイン',
        description: '登録したメールアドレスでログインしてください。',
        google: 'Google でログイン',
        submit: 'ログイン',
...
export function AuthForm({ mode, redirect }: AuthFormProps) {
    ...
    const onSubmit = async (input: SignInInput) => {
        ...
        await router.navigate({ href: redirectPath, replace: true })
    }
    ...
    return (
        <div className='flex min-h-dvh items-center justify-center bg-white px-6 py-8 md:bg-slate-50'>
            <div className='w-full max-w-sm space-y-5 md:rounded-2xl md:border md:border-slate-200 md:bg-white md:p-9'>
