...
describe('ComponentPropsPanel（props を持たないコンポーネント）', () => {
    it('コンポーネント名の下に「設定する項目はありません」と出す', () => {
        ...
        rerender(<ComponentPropsPanel block={{ id: '3', type: 'artistSummary', props: {} }} onChange={vi.fn()} />)
        expect(screen.getByRole('heading', { name: '出演者のサマリー' })).toBeInTheDocument()
        expect(screen.getByText('設定する項目はありません')).toBeInTheDocument()

        rerender(<ComponentPropsPanel block={{ id: '4', type: 'setList', props: {} }} onChange={vi.fn()} />)
