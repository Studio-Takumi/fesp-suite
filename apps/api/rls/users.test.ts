...
import { anonClient, createArticle, idsOf, serviceClient, sorted, useRlsFixture } from './support'

const f = useRlsFixture()

/**
 * イベントに所属していないユーザーを作成者にした記事を eventA に作る（イベントを抜けた人が書いた記事の代わり）。
 * 記事は staff の権限でしか作れないので、作る間だけ eventA の staff にする
 */
async function createArticleByOutsider(title: string, status: 'draft' | 'published'): Promise<string> {
    await serviceClient.from('event_members').insert({ user_id: f.outsider.id, event_id: f.eventA, role: 'staff' })
    try {
        return await createArticle(f.outsider, f.eventA, title, status)
    } finally {
        await serviceClient.from('event_members').delete().eq('user_id', f.outsider.id).eq('event_id', f.eventA)
    }
}
...
describe('users の読み取り', () => {
    ...
    it('イベントに所属していない作成者の行も、記事のイベントのメンバーは記事と一緒に読める', async () => {
        const articleId = await createArticleByOutsider('抜けた人の記事', 'published')
    ...
    it('下書きの記事の作成者は、記事を通しては staff にしか見えない', async () => {
        ...
        const articleId = await createArticleByOutsider('抜けた人の下書き', 'draft')
        ...
        await serviceClient.from('articles').delete().eq('id', articleId)
    })
})
