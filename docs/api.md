...
イベントのメンバーなら役割を問わず読める。イベントが存在しないとき、所属していないときも `items` が空の `200` を返す。
...
記事を1件、本文つきで取得する。イベントのメンバーなら役割を問わず読める。
...
記事のタイトルと本文を丸ごと置き換える。`updated_at` は現在時刻になる。記事のイベントの `staff` だけが更新できる。
...
| ステータス | `code`         | 条件                                                                                        |
| ---------- | -------------- | ------------------------------------------------------------------------------------------- |
| `400`      | `bad_request`  | `id` の形式が不正、`title` が100文字を超える、または `content` が記事ドキュメントの形でない |
| `401`      | `unauthorized` | `Authorization` ヘッダーがない、またはトークンが無効                                        |
