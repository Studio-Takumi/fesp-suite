# API仕様

## 記事（articles）

### 記事オブジェクト

| フィールド   | 型       | 説明                                               |
| ------------ | -------- | -------------------------------------------------- |
| `id`         | `uuid`   | 記事ID                                             |
| `event_id`   | `uuid`   | 所属するイベント                                   |
| `title`      | `string` | タイトル。100文字まで。空文字のこともある          |
| `content`    | `array`  | BlockNoteのブロック配列（`articleDocumentSchema`） |
| `created_at` | `string` | 作成日時                                           |
| `updated_at` | `string` | 更新日時                                           |

一覧では本文を返さないため、`content` を除いた形になる。

### `GET /api/articles`

イベントの記事一覧。`updated_at` の新しい順。

#### クエリ

| 名前       | 型       | 必須 | 説明                |
| ---------- | -------- | ---- | ------------------- |
| `event_id` | `uuid`   | ✅   | 対象のイベント      |
| `limit`    | `number` |      | 1〜100。既定値 `20` |
| `offset`   | `number` |      | 0以上。既定値 `0`   |

#### レスポンス（`200`）

```json
{
    "items": [
        {
            "id": "7f1c2a9e-3b4d-4e5f-8a6b-1c2d3e4f5a6b",
            "event_id": "0b7e6d5c-4a3b-4c2d-9e1f-a2b3c4d5e6f7",
            "title": "模擬店のお知らせ",
            "created_at": "2026-09-14T10:00:00+09:00",
            "updated_at": "2026-09-14T12:30:00+09:00"
        }
    ],
    "limit": 20,
    "offset": 0
}
```

イベントが存在しないときも `items` が空の `200` を返す。

#### エラー

| ステータス | `code`        | 条件         |
| ---------- | ------------- | ------------ |
| `400`      | `bad_request` | クエリが不正 |

### `GET /api/articles/:id`

記事を1件、本文つきで取得する。

#### パス・クエリ

| 名前       | 場所   | 型     | 必須 | 説明           |
| ---------- | ------ | ------ | ---- | -------------- |
| `id`       | パス   | `uuid` | ✅   | 記事ID         |
| `event_id` | クエリ | `uuid` | ✅   | 対象のイベント |

#### レスポンス（`200`）

```json
{
    "id": "7f1c2a9e-3b4d-4e5f-8a6b-1c2d3e4f5a6b",
    "event_id": "0b7e6d5c-4a3b-4c2d-9e1f-a2b3c4d5e6f7",
    "title": "模擬店のお知らせ",
    "content": [
        {
            "id": "a1",
            "type": "paragraph",
            "props": { "backgroundColor": "default", "textColor": "default", "textAlignment": "left" },
            "content": [{ "type": "text", "text": "現金のみです。", "styles": {} }],
            "children": []
        }
    ],
    "created_at": "2026-09-14T10:00:00+09:00",
    "updated_at": "2026-09-14T12:30:00+09:00"
}
```

#### エラー

| ステータス | `code`        | 条件                                     |
| ---------- | ------------- | ---------------------------------------- |
| `400`      | `bad_request` | `id` / `event_id` の形式が不正           |
| `404`      | `not_found`   | 記事が存在しない、または別イベントの記事 |

### `POST /api/articles`

記事を作成する。

#### クエリ

| 名前       | 型     | 必須 | 説明           |
| ---------- | ------ | ---- | -------------- |
| `event_id` | `uuid` | ✅   | 対象のイベント |

#### リクエストボディ

```json
{
    "title": "模擬店のお知らせ",
    "content": []
}
```

| フィールド | 型       | 必須 | 説明                                                         |
| ---------- | -------- | ---- | ------------------------------------------------------------ |
| `title`    | `string` | ✅   | タイトル。前後の空白を取り除いたうえで100文字まで。空文字可  |
| `content`  | `array`  | ✅   | BlockNoteのブロック配列（`articleDocumentSchema`）。空配列可 |

#### レスポンス（`201`）

作成した記事オブジェクト（`GET /api/articles/:id` と同じ形）。

#### エラー

| ステータス | `code`        | 条件                                                                                   |
| ---------- | ------------- | -------------------------------------------------------------------------------------- |
| `400`      | `bad_request` | クエリが不正、`title` が100文字を超える、または `content` が記事ドキュメントの形でない |
| `404`      | `not_found`   | イベントが存在しない                                                                   |

### `PUT /api/articles/:id`

記事のタイトルと本文を丸ごと置き換える。`updated_at` は現在時刻になる。

#### パス・クエリ

| 名前       | 場所   | 型     | 必須 | 説明           |
| ---------- | ------ | ------ | ---- | -------------- |
| `id`       | パス   | `uuid` | ✅   | 記事ID         |
| `event_id` | クエリ | `uuid` | ✅   | 対象のイベント |

#### リクエストボディ

`POST /api/articles` と同じ。

#### レスポンス（`200`）

更新後の記事オブジェクト（`GET /api/articles/:id` と同じ形）。

#### エラー

| ステータス | `code`        | 条件                                                                                                     |
| ---------- | ------------- | -------------------------------------------------------------------------------------------------------- |
| `400`      | `bad_request` | `id` / `event_id` の形式が不正、`title` が100文字を超える、または `content` が記事ドキュメントの形でない |
| `404`      | `not_found`   | 記事が存在しない、または別イベントの記事                                                                 |

## 自分（me）

### ユーザーオブジェクト

| フィールド   | 型       | 説明                          |
| ------------ | -------- | ----------------------------- |
| `id`         | `uuid`   | ユーザーID（`auth.users.id`） |
| `created_at` | `string` | 作成日時                      |
| `updated_at` | `string` | 更新日時                      |

### `GET /api/me`

ログイン中のユーザー自身を取得する。

#### ヘッダー

| 名前            | 必須 | 説明                                   |
| --------------- | ---- | -------------------------------------- |
| `Authorization` | ✅   | `Bearer <Supabase のアクセストークン>` |

#### レスポンス（`200`）

```json
{
    "id": "3c9d1e2f-4a5b-4c6d-8e7f-9a0b1c2d3e4f",
    "created_at": "2026-09-15T10:00:00+09:00",
    "updated_at": "2026-09-15T10:00:00+09:00"
}
```

#### エラー

| ステータス | `code`         | 条件                                                 |
| ---------- | -------------- | ---------------------------------------------------- |
| `401`      | `unauthorized` | `Authorization` ヘッダーがない、またはトークンが無効 |
| `404`      | `not_found`    | ユーザーの行が存在しない、または論理削除されている   |
