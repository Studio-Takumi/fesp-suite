...
| `apps/web/src/components/article/blocks/PageHeader.tsx`    | 追加。英語ラベル（Inter・大文字）と日本語タイトル（`h2`・Noto Sans JP）を `header` に出す       |
...
- **ページ見出しの日本語タイトルは `h2` にした（発注者と合意）。** 記事ページのタイトルがすでに `h1` のため。ページのタイトルとの整理は #29 で扱う
...
| `apps/web/src/components/article/ArticleRenderer.test.tsx`  | 英語ラベル（大文字・Inter）と日本語タイトル（`h2`）を `header` に出す、空の行を出さない、両方とも空なら `header` ごと出さない                                                      |
