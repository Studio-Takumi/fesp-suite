 ...
 *
 * `#5` 時点ではテキスト系ブロック（paragraph/heading/bulletListItem/numberedListItem）のみを
 * 対象にする。独自コンポーネントブロックは `#24` で追加する（docs/article-system.md 参照）。
...
const headingPropsSchema = blockPropsSchema.extend({
    level: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(6)]),
    isToggleable: z.boolean().optional(),
})

const numberedListItemPropsSchema = blockPropsSchema.extend({
...
export type ArticleBlock = {
    ...
    props:
        | z.infer<typeof blockPropsSchema>
        | z.infer<typeof headingPropsSchema>
        | z.infer<typeof numberedListItemPropsSchema>
    content: (ArticleStyledText | ArticleLink)[]
